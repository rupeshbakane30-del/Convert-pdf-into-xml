...
CREATE OR REPLACE PACKAGE BODY PKG_DASHBOARD AS
  ...
  BEGIN
    SELECT COUNT(*) INTO p_total_filings FROM GST_FILINGS WHERE user_id = p_user_id;
    SELECT COUNT(*) INTO p_total_conv    FROM BANK_CONVERSIONS WHERE user_id = p_user_id AND status = 'COMPLETED';
